...
if [ $cip = $(cat $dns_ip_file) ]; then
  ...
  for api_id in $(cat $dns_file | jq '.result | .[].id' | sed 's/\"//g'); do
    ...
    curl --silent -X PUT "${api_host_dns}/${api_id}" -H "$ca_mail" -H "$ca_key1" -H "$ca_ct" \
      --data "{\"type\":\"AAAA\",\"name\":\"${api_name}\",\"content\":\"${cip}\",\"ttl\":${api_ttl},\"proxied\":${api_proxy}}" | tee $dns_log | jq
  done
...
curl --silent -X GET "${api_host_dns}?type=A&content=2606:4700:4700::1111" -H "$ca_mail" -H "$ca_key1" -H "$ca_ct" > $dns_file
