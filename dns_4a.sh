#!/bin/bash
#####
#Change AAAA DNS records in cloudflare like DynDNS
#####
...
if [[ "$cip" == "$(cat $dns_ip_file)" ]]; then
  ...
  curl --silent -X GET "${api_host_dns}?type=AAAA&content=$(cat $dns_ip_file)" -H "$ca_key1" -H "$ca_ct" > $dns_file
  for api_id in $(cat $dns_file | jq '.result | .[].id' | sed 's/\"//g'); do
    ...
    curl --silent -X PUT "${api_host_dns}/${api_id}" -H "$ca_key1" -H "$ca_ct" \
      --data "{\"type\":\"AAAA\",\"name\":\"${api_name}\",\"content\":\"${cip}\",\"ttl\":${api_ttl},\"proxied\":${api_proxy}}" | tee -a $dns_log | jq
  done
...
curl --silent -X GET "${api_host_dns}?type=AAAA&content=2606:4700:4700::1111" -H "$ca_key1" -H "$ca_ct" > $dns_file
